...
class seventest(unittest.TestCase):
    ...
    def test_seventestdetail(self):
        ...
        logging.info("Email All pending加载时间"+ str(durtime1 / 1000)+".s")
        sleep(5)
        # 定位到Sent Report文件夹
        ...
        # 定位到关键字搜索框
        driver.find_element_by_xpath(".//*[@id='keyword']").send_keys("Mark Rovera")
        sleep(5)
        # 定位到Search 按钮
        ...
        driver.find_element_by_xpath(".//*[@id='listDiv']//td[contains(text(),'Mark Rovera')]").click()
        ...
        logging.info("Email无附件详情页面加载时间"+ str(durtime6 / 1000)+ ".s")
        sleep(20)
        # 顶层
        ...
        driver.switch_to.frame("ctn-iframe")
        # 定位到关键字搜索框
        driver.find_element_by_xpath(".//*[@id='keyword']").send_keys("Lilian Lilian")
        sleep(5)
        # 定位到Search 按钮
        driver.find_element_by_xpath(".//*[@id='btn-search']").click()
        sleep(20)
        navigationStart5 = driver.execute_script("return window.performance.timing.fetchStart")
        driver.find_element_by_xpath(".//*[@id='listDiv']//td[contains(text(),'Lilian Lilian')]").click()
        ...
        logging.info("Email有附件详情页面加载时间"+str(durtime5 / 1000)+ ".s")
        sleep(5)
        # 顶层
        driver.switch_to.default_content()
        # 定位到Message菜单
        ...
        logging.info("Message All pending加载时间"+str(durtime2 / 1000)+ ".s")
        sleep(10)
        # 定位到Starred文件夹
        ...
        logging.info("Message无附件详情页面加载时间"+ str(durtime8 / 1000)+ ".s")
        sleep(5)
        # 顶层
        driver.switch_to.default_content()
        # iframe_message层
        ...
        logging.info("Message有附件详情页面加载时间"+ str(durtime8 / 1000)+ ".s")
        sleep(5)
        #顶层
        driver.switch_to.default_content()
        # 定位到Ticket模块
        try:
            WebDriverWait(driver, 10).until(EC.presence_of_element_located(
                (By.XPATH, './/*[@id=\'col-main\']/div[1]/div[3]/ul/li[6]/a'))).send_keys(Keys.ENTER)
        except Exception as e:
            print("找不 id为 create_link " + e)
        driver.implicitly_wait(30)
        ...
        logging.info("Ticket All加载时间"+str(durtime3 / 1000)+ ".s")
        sleep(5)
        # 定位到Close文件夹
        driver.find_element_by_xpath(".//*[@id='col_sub']/div[3]/ul/li[9]/a").click()
        # ctn-iframe层
        driver.switch_to.frame("ticket-list")
        sleep(5)
        # 定位到关键字搜索框
        driver.find_element_by_xpath(".//*[@id='keyword']").send_keys("NTT237609")
        sleep(5)
        # 定位到Search 按钮
        driver.find_element_by_xpath(".//*[@id='btn-search search_1']").click()
        sleep(20)
        navigationStart20 = driver.execute_script("return window.performance.now()")
        driver.find_element_by_xpath(".//*[@id='listDiv']//td[contains(text(),'quality')]").click()
        loadEventEnd20 = driver.execute_script("return window.performance.now()")
        durtime20 = (loadEventEnd20 - navigationStart20)
        print("Ticket有附件详情页面加载时间", str(durtime20 / 1000), ".s")
        logging.info("Ticket有附件详情页面加载时间" + str(durtime20 / 1000) + ".s")
