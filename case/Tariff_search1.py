...
from case.Tariff_search import *
...
class ProductCategory(unittest.TestCase):
    ...
    def variable(self):
        ...
        global results
        file = open(rootPath + '/data/test_ProductCategory.txt', encoding='utf-8')
        data = json.load(file)
        results = [(d['key']) for d in data['variable']]
    ...
    def test_ProductCategory(self):
        ...
        # 定位到搜索框
        self.driver.find_element_by_xpath("//*[@id='TariffSearchFormPanelID']//input[contains(@class,'x-form-text')]").send_keys(we)
