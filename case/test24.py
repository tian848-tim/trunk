...
class purchasePlan(unittest.TestCase):
    ...
    def quotation(self):
        ...
        file = open(rootPath + '/data/workflow_PurchasingClass_purchaseplan_flowPurchasePlan.json', encoding='utf-8')
        data = json.load(file)
        results = [(d['key']) for d in data['quotation']]
    ...
    def keywords(self):
        ...
        file = open(rootPath + '/data/workflow_PurchasingClass_purchaseplan_flowPurchasePlan.json', encoding='utf-8')
        data = json.load(file)
        results = [(d['key']) for d in data['keywords']]
    ...
    def leadTime(self):
        ...
        file = open(rootPath + '/data/workflow_PurchasingClass_purchaseplan_flowPurchasePlan.json', encoding='utf-8')
        data = json.load(file)
        results = [(d['key']) for d in data['leadTime']]
    ...
    def test_PurchasePlan(self):
        ...
        if we[0] != '':
            ...
            # 定位到采购询价
            self.driver.find_element_by_xpath("//*[@id='FlowPurchasePlanForm']//span[contains(text(),'采购询价列表')]").click()
            ...
            if  er[0] != '':
                       ...
                       _elementFiveth = (random.randint(1, 1))
