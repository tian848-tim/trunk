...
class FeeRegister(unittest.TestCase):
    ...
    def variable(self):
        ...
        try:
            global ABC,variable
            file = open(rootPath + '/data/'+ "test_CustomClearance" + time.strftime("%Y-%m-%d") + '.json', encoding='utf-8')
            data = json.load(file)
            ...
            print("test_CustomClearance" + time.strftime("%Y-%m-%d") + '.json'+"File not found")
    ...
    def setUp(self):
        ...
        # 脚本标识－标题
        self.script_name = '费用登记-组合'
        # 脚本标识－ID
        self.script_id = 'workflow_FinancialClass_feeregister_FlowFeeRegister_grouping'
    ...
    def test_flowFeeRegister(self):
        ...
        # 定位到费用登记搜索
        self.driver.find_element_by_xpath("//*[@id='FlowFeeRegistrationView-body']//input[@name='keywords']").send_keys(we[0])

        sleep(2)

        # 定位到费用登记搜索
        self.driver.find_element_by_xpath("//*[@id='FlowFeeRegistrationView-body']//span[contains(@class, 'fa-search')]").click()
        ...
        ul = self.driver.find_element_by_xpath("//*[@id='FlowFeeRegistrationGridPanelID-body']/div/table/tbody/tr[1]")
