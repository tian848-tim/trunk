...
class purchasePlan(unittest.TestCase):
    ...
    def loadvendername(self):
        ...
        global result
        file = open(rootPath + '/data/workflow_PurchasingClass_purchaseplan_flowPurchasePlan_WrittenDocuments.json', encoding='utf-8')
        data = json.load(file)
    ...
    def setUp(self):
        # 脚本标识－标题
        self.script_name = '采购计划-冲销金额'
        # 脚本标识－ID
        self.script_id = 'workflow_PurchasingClass_purchaseplan_flowPurchasePlan_WrittenDocuments'
    ...
    def test_PurchasePlan(self):
        ...
        try:
             self.driver.find_element_by_xpath("//*[@id='FlowPurchasePlanFormPanelID']//div[contains(text(),'{}')]".format('没有数据！')).is_displayed()
             a = True
        except:
             a = False
        if a == True:
